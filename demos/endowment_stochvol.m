% ENDOWMENT_STOCHVOL  Small Open Endowment Economy with Incomplete Markets
...
%% Cadena de Markov de y
 ny = 4;
 p  = .8;          % probability of staying in low output
 q  = .8;          % probability of staying in high output
 pL = 0.9;         % probability of staying in low volatility regime
 pH = 0.8;         % probability of staying in high volatility regime
 ym = 1;           % output mean
 e  = 0.2;         % scalar, size of epsilons: ymean+/-epsilon 
 a  = 1;
 
 [y1,PI1] = markovchain(ny,p,q,e,ym) % standard Markov process
 [y2,PI2] = svmc(p,q,pL,pH,e,ym)     % stochastic volatility Markov process
...
% Plots stationary distributions 
 figure(1); plot(B,[F1 F2]);     
 legend('standard','stochastic volatility')
 figure(2); plot(B,[pie1 pie2]); 
 legend('standard','stochastic volatility')
...
display('Means & Std.Devs of Standard vs Asymmetric Endowment Model')
