% AGUIAR-GOPINATH Aguiar & Gopinath (2006) Model I by state space discretization
% The present code solves the A&G model for transitory shocks, stable trend and exogenous labor
...
sigma_z  = 0.034;                              % standard deviation of shocks for growth rate
mn_z     = (-1/2)*(sigma_z)^2;                 % long run mean for trend income
rho_g    = 0.17;                                % autocorrelation coefficient                                 0.17
sigma_g  = 0.0;                                % standard deviation of shocks for growth rate                0.03
...
bmin   = -0.3;                                 % min value of asset grid  
...
if sigma_g >0
[gprob,g] = appmarkov2(cons_g,rho_g,sigma_g,width,n1);
trend = exp(g);
else 
trend = mn_g;     
end
...
[B,Z,TREND] = gridmake(b,z,trend);                      % the order of states follows A&G code
Y     = A*exp(Z).*TREND;
...
fprintf('\nAGUIAR-GOPINATH STABLE TREND MODEL     \n') 
for it = 1:maxit
...
[vdef,xdef,vpay,xpay,V,D] = solvedpos(upay,udef,zprob,beta,lambda,bzero);

% 9. Compute the expected value of default

Edef = zprob*(reshape(D,m,n1)');
ind = reshape(D,m,n1)';
...
Pstar   = otpm1v(x,zprob,n2,n1);
pie     = ergdist(Pstar);                                       % ergodic distribution
Eq      = (pie'*q)';                                              
Ebp_bar = pie'*b_bar(x);
Ebp     = pie'*b(x);

% Before simulating the model, define an approximation space for the bond
% price
