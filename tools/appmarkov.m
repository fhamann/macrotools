function [Tran,s,probst,alambda,asigmay]=appmarkov(lambda,sigma,m,N)

% APPMARKOV approximates a zero-mean AR(1) process with Markov chain
%
...
for j=1:N;
   for k=2:N-1;
         Tran(j,k)= cdfnorm(s(k)-lambda*s(j)+w/2,0,sigma)...
         - cdfnorm(s(k)-lambda*s(j)-w/2,0,sigma);
   end
   Tran(j,1) = cdfnorm(s(1)-lambda*s(j)+w/2,0,sigma);
   Tran(j,N) = 1 - cdfnorm(s(N)-lambda*s(j)-w/2,0,sigma);   
end
