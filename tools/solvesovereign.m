function [vs,xs,vc,xc,D,V] = solvesovereign(fc,fs,prob,beta,lambda,i0)

% SOLVESOVEREIGN  Solves sovereign default problem by discretization
%
% Usage:
%
%   [vs,xs,vc,xc,D,V] = solvesovereign(fc,fs,prob,beta,lambda,i0)
%
...
for it=1:maxit
    ...
    [vs,xs] = max(fs+lambda*(beta.*EV0)+(1-lambda)*(beta.*Evs),[],2);
    [vc,xc] = max(fc+beta.*EV,[],2);
...
if change>tol,warning('No convergence'); end
    ...
    P  = kron(speye(m,m),reshape(permute(prob,[1 3 2]),n,n/m));
...
for it=1:maxit
    ...
    Evc = reshape(P*vc,n,m);
    Evs = reshape(P*vs,n,m);
    EV  = reshape(P*V,n,m);
    EV0 = reshape(P*v0,n,m);
